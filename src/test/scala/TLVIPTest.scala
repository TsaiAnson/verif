...
//  }
  ...
  it should "Basic Unittest Burst TLTransaction to TLBundle conversion" in {
    val results = TLTransactiontoTLBundles(PutFullBurst(addr = 0x0.U, masks = List(0xff.U, 0x7f.U),
  ...
  it should "Basic Unittest TLTransaction to TLBundles to TLTransaction" in {
    val testTxns = List(AccessAck(size = 3.U, denied = true.B), AccessAckData(size = 3.U, denied = false.B, data = 0x1.U(64.W)),
      ...
      PutFull(addr = 0x10.U, mask = 0xff.U, data = 0x11.U(64.W)),
      PutFullBurst(size = 4.U, addr = 0x0.U, masks = List(0xff.U, 0x7f.U), datas = List(0x1234.U(64.W), 0x9876.U(64.W))),
      Get(size = 3.U, addr = 0x15.U, mask = 0xff.U), Get(size = 4.U, addr = 0x20.U, mask = 0xff.U)
    )
    var i = 0
    for (txn <- testTxns) {
